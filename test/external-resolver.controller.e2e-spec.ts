...
describe("ExternalResolverController (e2e)", () => {
    ...
    it("should create, get, and resolve an external resolver set.", async () => {
        ...
        await request(app.getHttpServer()).get(`/external-resolvers/${id}`).expect(200).then((response) => {
            expect(response.body).toEqual(
                expect.objectContaining({
                    id,
                    pattern: `^(N|3|S|Q|W|NA75|M|T).*`,
        ...
        // Resolve NSW PIC 
        await request(app.getHttpServer()).get('/NSW123456').expect(302)
            .expect("Location", 'https://nsw-pic.com/NSW123456');

        // Resolve SA PIC 
        await request(app.getHttpServer()).get('/SA123456').expect(302)
